...
from kitti_dataset import Kitti2DDetection,collate_fn


parser = argparse.ArgumentParser()
parser.add_argument("--model_dir",default='checkpoints',type=str)
parser.add_argument("--data_dir",default='data',type=str)
parser.add_argument("--model_name",default='bagnet33_kitti.pth',type=str)
...
train_dataset = Kitti2DDetection(DATA_DIR,image_set='train')
...
if 'bagnet17' in args.model_name:
    ...
    FM_SIZE = (26,91) #hard-code the feature map size for now 
    RF_SIZE = 17
...
def criterion(logits_list, targets):
    ...
    for i in range(B):
        ...
        boxes = tar['bbox']
        if len(boxes) > 0 :
            boxes = bboxes_img2fm(boxes,FM_SIZE,RF_SIZE)#.astype(int)
            #boxes[:,:2]+=1
            #boxes[:,2:]-=1
            #boxes = np.clip(boxes,0,FM_SIZE)
            boxes[:,2] = np.clip(boxes[:,2],0,FM_SIZE[0])
            boxes[:,3] = np.clip(boxes[:,3],0,FM_SIZE[1])
            ########################################################################
            logits = torch.clamp(logits_list[i],0,torch.tensor(float('inf'))) 
            labels = tar['labels'].cuda()
            reduced_logits = []
            mask  = torch.ones(size=(FM_SIZE[0],FM_SIZE[1]),dtype=bool).cuda()
            for j,xyxy in enumerate(boxes):
                ...
                mask[xyxy[0]:xyxy[2],xyxy[1]:xyxy[3]]=False

            reduced_logits = torch.stack(reduced_logits)
            fil = ~torch.isnan(reduced_logits[:,0])
            #print(reduced_logits.shape)
            #print(fil)
            reduced_logits = reduced_logits[fil]
            #print(reduced_logits.shape)
            #print()
            labels = labels[fil]
            logits_neg = torch.mean(logits.permute(1,2,0)[mask.cuda()],dim=0,keepdim=True)
            tmp = torch.nn.functional.cross_entropy(reduced_logits,labels)
            loss_pos += tmp
            if torch.isnan(tmp):
                print(reduced_logits.shape,labels.shape,tmp)
            ...
            #print(torch.argmax(reduced_logits,dim=1) == labels)
